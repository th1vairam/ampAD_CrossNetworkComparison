...
thisFileName = 'enrichSpeakEasyModules.R'

# Github link
thisRepo <- getRepo(repository = "/th1vairam/ampAD_CrossNetworkComparison", 
                    ref="branch", 
                    refName='enrich')
...
activityName = 'Fisher enrichment with enrichr genesets'
activityDescription = 'Enrichment analysis of network modules from SpeakEasy using Fishers method'
############################################################################################################
...
annotate = annotations(MOD_OBJ)
FNAME = paste(annotate$brainRegion, annotate$disease, 'Enrichment', 'METANETWORK')
...
backGroundGenes = unique(MOD$hgnc_symbol)
...
enrichResults = list()
for (name in unique(MOD$modulelabels)[1:4]){
